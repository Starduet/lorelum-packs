...
搜索框持有 `query`，旁边的同级结果摘要也持有一份由 props 初始化的 `query`。一个 Effect 把每次按键都复制进摘要的状态。任何一侧变化时两个值都可能分叉，而这段额外同步为一个概念制造了第二个所有者。
...
- 父组件已经通过 `value` 与 `onChange` 控制子组件时，父组件就是所有者；除非有意建立另一种本地覆盖契约，子组件不应再造第二份可编辑副本。
- 许多遥远的后代需要同一个值时，Context 可以减少 props 传递，但要慎重选择 provider 的作用范围。本篇不规定 compound-component API、状态管理库或服务器数据缓存。
